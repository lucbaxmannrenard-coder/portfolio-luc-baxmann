/* From oso95/scroll-world (https://github.com/oso95/scroll-world), MIT License. */
/* ============================================================================
...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      // Keep the CSS -50% vertical centering: a bare translateY() here overrides the
      // class transform and pushes the copy block below the fold.
      c.style.transform = reduce ? 'translateY(-50%)' : `translateY(calc(-50% + ${((0.5 - pr) * 4).toFixed(2)}vh))`;
...
function injectCSS() {
  ...
  /* No pointer-events override here: the CTA must follow its copy's inline
     pointer-events (set per-frame in read()), or an invisible CTA stays clickable. */
